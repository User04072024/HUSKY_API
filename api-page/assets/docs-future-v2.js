...
function prettifyEndpointName(path = "", summary = "") {
  ...
  const segment = String(path).split("/").filter(Boolean).pop() || "Endpoint";
  return segment
    .split(/[-_]/g)
...
function normalizeCategories(spec) {
  ...
  Object.entries(spec.paths || {}).forEach(([path, methods]) => {
    Object.entries(methods || {}).forEach(([method, endpoint]) => {
      ...
      const summary = endpoint.summary || "Sin resumen";

      map[tagName].items.push({
        path,
        method: String(method).toUpperCase(),
        displayName: prettifyEndpointName(path, summary),
        summary,
                  ...
                  (item, index) => `
                    <article class="future-endpoint-card">
                      ...
                      >
                        ...
                        <div class="future-endpoint-path break-anywhere">${escapeHtml(item.displayName)}</div>
