...
const endpointTranslations = {
  ...
  '/download/pinterest': { summary: 'Pinterest', description: 'Obtiene datos y enlaces multimedia desde una URL de Pinterest o pin.it.' },
  '/download/tiktok': { summary: 'TikTok', description: 'Descarga video de TikTok.' },
  ...
  '/image/wallpaper': { summary: 'Wallpaper', description: 'Imágenes aleatorias desde WallpaperFlare según una búsqueda.' },
...
function normalizeCategories(spec) {
  ...
  return Object.values(map).map(category => ({ ...category, items: category.items.sort((a,b)=>a.path.localeCompare(b.path)) })).filter(category => category.items.length > 0).sort((a,b)=>a.name.localeCompare(b.name));
}
...
function renderApiList() {
  ...
  const filteredCategories = state.categories.map(category => ({ ...category, items: category.items.filter(item => [category.name,item.path,item.summary,item.description,item.method].join(' ').toLowerCase().includes(searchTerm)) })).filter(category => category.items.length > 0);
...
function buildFields(endpoint) {
  ...
  (endpoint.parameters || []).forEach(param => {
    ...
    if (Array.isArray(schema.enum) && schema.enum.length) {
      parts.push(`<div><label class="block text-xs font-semibold text-slate-700 mb-2">${label}</label><select name="${escapeAttr(param.name)}" class="w-full border border-slate-200 bg-white px-3 py-3 text-sm focus:outline-none focus:border-slate-500" ${required ? 'required' : ''}>${schema.enum.map(value => `<option value="${escapeAttr(value)}">${escapeHtml(value)}</option>`).join('')}</select>${helper}</div>`);
    } else {
      const inputType = (schema.type === 'number' || schema.type === 'integer') ? 'number' : 'text';
      parts.push(`<div><label class="block text-xs font-semibold text-slate-700 mb-2">${label}</label><input type="${inputType}" name="${escapeAttr(param.name)}" placeholder="${placeholder}" class="w-full border border-slate-200 bg-white px-3 py-3 text-sm focus:outline-none focus:border-slate-500 placeholder:text-slate-400" ${required ? 'required' : ''} />${helper}</div>`);
    }
  });
  if (!parts.length) {
    return `<div class="border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Este endpoint no requiere parámetros.</div>`;
  }
  return parts.join('');
}

function buildCurl(method, url) { return `curl -X ${method} "${url}"`; }
...
async function executeEndpoint(event) {
  ...
  curlBox.textContent = buildCurl(method, fullUrl);
...
endpointModal.addEventListener('click', (event) => { if (state.currentEndpoint && modalCard && !modalCard.contains(event.target)) closeModal(); });
window.toggleCategory = toggleCategory;
window.openEndpointByFilter = openEndpointByFilter;
document.addEventListener('DOMContentLoaded', () => { loadNotifications(); loadApis(); });
