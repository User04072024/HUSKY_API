...
function drawStatusImage(metrics) {
    if (!createCanvas || process.env.STATUS_IMAGE_FORMAT !== "png") {
        ...
        const cards = [["CPU PROCESS", `${metrics.cpuPercent || 0}%`, Math.min(100, metrics.cpuPercent || 0)], ["MEMORIA", `${metrics.memory || 0} MB`, Math.min(100, ((metrics.memory || 0) / 1024) * 100)], ["REQUESTS / MIN", String(metrics.requestsPerMinute), Math.min(100, (metrics.requestsPerMinute || 0) * 4)], ["AVG LATENCY", `${metrics.averageLatency} ms`, Math.min(100, (metrics.averageLatency || 0) / 10)], ["ERROR RATE", `${metrics.errorRate}%`, Math.min(100, metrics.errorRate || 0)], ["ENDPOINTS", String(metrics.endpointCount || 0), Math.min(100, ((metrics.endpointCount || 0) / 20) * 100)]];
        const cardMarkup = cards.map(([label, value, percent], index) => { const x = 72 + (index % 3) * 360; const y = 290 + Math.floor(index / 3) * 126; const barWidth = Math.max(4, Math.round(276 * percent / 100)); return `<rect x="${x}" y="${y}" width="320" height="92" rx="3" fill="#0b1828" stroke="#164e63"/><text x="${x + 22}" y="${y + 28}" fill="#67e8f9" font-family="Arial, sans-serif" font-size="15" font-weight="700">${escapeXml(label)}</text><text x="${x + 22}" y="${y + 63}" fill="#f8fafc" font-family="Arial, sans-serif" font-size="28" font-weight="700">${escapeXml(value)}</text><rect x="${x + 22}" y="${y + 76}" width="276" height="5" rx="2" fill="#172333"/><rect x="${x + 22}" y="${y + 76}" width="${barWidth}" height="5" rx="2" fill="#22d3ee"/>`; }).join("");
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#070b13"/><stop offset="1" stop-color="#0b1828"/></linearGradient><pattern id="grid" width="48" height="48" patternUnits="userSpaceOnUse"><path d="M 48 0 L 0 0 0 48" fill="none" stroke="#164e63" stroke-opacity=".22"/></pattern></defs><rect width="1200" height="630" fill="url(#bg)"/><rect width="1200" height="630" fill="url(#grid)"/><text x="72" y="70" fill="#20d3ee" font-family="Arial, sans-serif" font-size="22" font-weight="700">HUSKY API // PROCESS MONITOR</text><text x="72" y="142" fill="#f8fafc" font-family="Arial, sans-serif" font-size="58" font-weight="700">OPERATIONAL</text><text x="76" y="184" fill="#94a3b8" font-family="Arial, sans-serif" font-size="20">${escapeXml(metrics.totalRequests)} total requests · ${escapeXml(metrics.uptimePercent)}% uptime · ${escapeXml(Math.floor((metrics.uptimeSeconds || 0) / 60))} min online</text>${cardMarkup}<text x="72" y="586" fill="#64748b" font-family="Arial, sans-serif" font-size="15">Generated ${escapeXml(new Date().toISOString())} // CPU + MEMORY + TRAFFIC + LATENCY</text></svg>`;
